...
export const CheckboxCard = React.forwardRef<HTMLInputElement, CheckboxCardProps>(
  ({ children, className, cardClassName, inputClassName, ...props }, ref) => {
    ...
    const classes = {
      label: cn(styles['checkbox-card'], className),
      card: cn(styles['checkbox-card__card'], cardClassName),
      input: cn(styles['checkbox-card__input'], inputClassName),
    };
    ...
    return (
      <label htmlFor={id} className={classes.label}>
        ...
        <div className={classes.card}>
          <button className={styles['checkbox-card__button']}>
            <svg
              width="15"
              height="15"
              viewBox="0 0 15 15"
